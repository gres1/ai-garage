...
const TOOLS = [
  ...
  { name: "register_remote", description: "Add a REMOTE server over SSH to AI Garage as a controllable card — the user can then start/stop the remote service from the panel with no terminal. Use when the user describes a VPS/remote box they want to manage (often by voice: 'add my server at 1.2.3.4, user root, start command X'). Pass host, user, the SSH key name from ~/.ssh, and the start/stop commands to run ON the server. The panel builds a safe ssh-wrapped command; keys stay by reference, never copied.", inputSchema: { type: "object", properties: { name: { type: "string", description: "friendly card name" }, host: { type: "string", description: "server IP or hostname" }, user: { type: "string" }, sshPort: { type: "number", description: "SSH port, default 22" }, keyPath: { type: "string", description: "path to the SSH private key, e.g. ~/.ssh/id_ed25519" }, startCmd: { type: "string", description: "command to run ON the server to start the service" }, stopCmd: { type: "string" }, url: { type: "string" } }, required: ["name", "host", "user", "startCmd"] } },
...
async function call(name, args) {
  ...
  if (name === "register_remote") return api("POST", "/api/remote-add", { name: args.name, host: args.host, user: args.user, sshPort: args.sshPort, keyPath: args.keyPath, startCmd: args.startCmd, stopCmd: args.stopCmd, url: args.url });
